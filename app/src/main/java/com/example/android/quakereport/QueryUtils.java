...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            // Create JSONObject using the sample response
            JSONObject sampleJsonResponse = new JSONObject(SAMPLE_JSON_RESPONSE);

            // Extract JSONArray for key 'features' to get list of features
            JSONArray earthquakeArray = sampleJsonResponse.getJSONArray("features");

            /**
             * Loop through the features to get needed data for {@link Earthqauke} object
             */
            for (int i=0; i < earthquakeArray.length(); i++) {
                // Get single earthquake at position 'i'
                JSONObject currentEarthquake = earthquakeArray.getJSONObject(i);

                // For each earthquake, extract JSONObject for key 'properties'
                JSONObject properties = currentEarthquake.getJSONObject("properties");

                // Extract magnitude from 'mag'
                ...
                // Create new {@link Earthquake} with collected data from the API
                Earthquake earthquake = new Earthquake(magnitude, location, time, url);

                // Add new earthquake to list of earthquakes
